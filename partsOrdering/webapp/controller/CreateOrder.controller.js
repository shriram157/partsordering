						...
						$.ajax({
									...
									success: function (data) {
										...
										$.each(data, function (i, item) {
												...
												var oModel = that.getModel(CONT_ORDER_MODEL); oModel.setProperty("/items", data1);
												var oOrderData = that.getView().getModel("orderModel").getData(); oOrderData.items.splice(0, 1);
												for (var i = 0; i < oOrderData.items.length; i++) { //changes by swetha for DMND0004095
													that.aCreateItems.push(oOrderData.items[i]);
												}
												oOrderData.items.splice(0, 0, that._getNewItem()); that.onDialogClose(); oOrderData.modifiedOn = new Date(); that.oOrderModel
												.setData(oOrderData); DataManager.setOrderData(oOrderData); //changes by swetha for DMND0004095
											},
											error: function (data) {
												console.log("I am inside error function");
												sap.m.MessageToast.show("Error,Please try again");
									...
									success: function (data) {
										...
										for (var i = 0; i < data.length; i++) {
											if (data[i].status == "Success") {
												...
												MessageBox.error(data[i].failureReasons[0].value + "" + "for" + "" + "Campaign Code" + "" + data[i].campaignCode.slice(3,
													6), {
													onClose: function (sAction) {}
												});
							...
							handleSortError: function (oEvent) {
								...
								this.itemTable.sort(oErrorStatusColumn, this._bSortColumnDescending ? sap.ui.table.SortOrder.Descending : sap.ui.table.SortOrder
									.Ascending, /*extend existing sorting*/
									true);
